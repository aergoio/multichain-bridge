...
function swapout_burn(token, amount, recipient)
  ...
  contract.call(token, "burnFrom", recipient, amount)
...
function swapout_transfer(token, amount, recipient)
  ...
  -- transfer the tokens from the user to the bridge
  contract.call(token, "transferFrom", recipient, system.getContractID(), amount)
...
function tokensReceived(operator, from, amount)
  ...
  -- ensure that the operator is this bridge contract
  assert(operator == system.getContractID(), "invalid operator")
